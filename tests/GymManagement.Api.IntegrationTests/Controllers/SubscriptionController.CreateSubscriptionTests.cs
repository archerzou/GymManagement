...
public class CreateSubscriptionTests
{
    ...
    public async Task CreateSubscription_WhenValidSubscription_ShouldCreateSubscription(SubscriptionType subscriptionType)
    {
        ...
        // Act
        HttpResponseMessage response = await _client.PostAsJsonAsync("Subscriptions", createSubscriptionRequest);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Created);
        response.Headers.Location.Should().NotBeNull();

        SubscriptionResponse? subscriptionResponse = await response.Content.ReadFromJsonAsync<SubscriptionResponse>();
